...
class multiregexp(Reader):
    ...
    def __init__(self, config, name, input_files):
        ...
        super(multiregexp, self).__init__(config, name)

        _multiregexp = self.config.get(self.name, "regexp")
        ...
        try:
            self._multiregexp = [re.compile(_multiregexp)]
            self.long_multiregexp = False
        except:
            ...
            self.long_multiregexp = True
            self._multiregexp = []
            splitmultiregexp = re.compile("[^)]*\(\?P.*?\)[^(]*")
            groups = splitmultiregexp.findall(_multiregexp)
            while groups:
                self._multiregexp.append(re.compile("".join(groups[0:99])))
                del groups[0:99]
    ...
    def next(self, extra_data = None):
        ...
        # 100 groups multiregexp workaround 
        subline = self.line
        result = []
        data = {}
        for submultiregexp in self._multiregexp:
            subresult = submultiregexp.search(self.line) 
            if subresult:
