...
##        self.r_type = ""
        ...
        if mysql_definition:
            ...
            self.transformers = transform_factory('transformers/', force_output_writer = 'mysql', nullable = True) 

    def mysql2field(self, mysql_field_string):
        import re
        regexp = re.compile("([A-Z]+)(\(([0-9]+)(?:,([0-9]+))?\))?(?:\s+(\w+))")
        # Improve
        # >>> regexp.findall("VARCHAR(3,70) unsigned zerofill")
        # [('VARCHAR', '(3,70)', '3', '70', 'unsigned')]

        res = regexp.findall(mysql_field_string.upper().strip()) 
        if res:
            field_type = res[0][0]
##            field_size = res[0][2]
##            field_decimal = res[0][3]
            field_unsigned = res[0][4]
##            self.r_unsigned = field_unsigned
            column_type = "_".join((field_type.lower(), field_unsigned.lower()))

            self.transformers = transform_factory('transformers/', force_output_type = column_type, force_output_writer = 'mysql')
...
##            return ""
        return "`%s` %s" % (self.field_name, self.transformers.get_best_definition())
##        return "`%s` %s %s %s" % (self.field_name, self.r_type, self.r_unsigned, self.r_null)
